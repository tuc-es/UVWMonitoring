...
class UVW:
    ...
    def addUVWConjunctively(self, other):
        offset = len(self.stateNames)
        for sn in range(len(other.stateNames)):
            self.stateNames.append(other.stateNames[sn])
            self.rejecting.append(other.rejecting[sn])
            self.transitions.append([(a+offset,b) for (a,b) in other.transitions[sn]])
        for i in other.initialStates:
            self.initialStates.append(i+offset)
    ...
    def restrictToTheCaseThatThereCanOnlyBeOneActionAtATime(self):
        ...
        for fromStateNo in range(0,len(oldTransitions)):
            ...
            self.transitions[fromStateNo] = t
    ...
    def getBottomUpAndReverseBottomUpOrder(self):
        ...
        while len(bottomUpOrder)!=len(self.stateNames):
            for i in range(0,len(self.stateNames)):
                allDone = reduce(lambda x,y: x and y, [a in bottomUpOrder or a==i for (a,b) in self.transitions[i]],True)
    ...
    def bruteForceStateRemoval(self):
        """This function tries to remove every state one-by-one and checks after each removal if the new UVW is still 
           bisimulation-equivalent to the original one. It does so by disconnecting the states. Quite expensive to call."""
    
        removed = set([])

        # See which ones we can remove    
        for stateNum in range(1,len(self.stateNames)):
            
            uvw1 = UVW(self.ddMgr)
            uvw1.stateNames = copy.copy(self.stateNames)
            uvw1.rejecting = copy.copy(self.rejecting)
            uvw1.transitions = copy.copy(self.transitions)
            uvw1.initialStates = copy.copy(self.initialStates)
            uvw1.propositions = copy.copy(self.propositions)
            
            # Modify: removed states are not initial and do not have incoming transitions
            removedCandidate = set(removed)
            removedCandidate.add(stateNum)
            for a in removedCandidate:
                if a in uvw1.initialStates:
                    uvw1.initialStates.remove(a)
                    
            # Only try bisimulation equivalence if there is an initial state left
            # --> Otherwise the simulation relation computation fails.
            # We're assuming here that states from which no self-looping states can be reached
            # have already been removed.
            for i in range(0,len(uvw1.transitions)):
                nova = []
                for (a,b) in uvw1.transitions[i]:
                    if not a in removedCandidate:
                        nova.append((a,b))
                uvw1.transitions[i] = nova

            if len(uvw1.initialStates)>=1:                                        
                if self.isBisimulationEquivalent(uvw1,self):
                    self.initialStates = uvw1.initialStates
                    self.transitions = uvw1.transitions
    ...
    def getBDDFromPropositionName(self,propName):
        #uvw = UVW()
        if not propName in self.propositions:
            self.propositions.append(propName)
            self.ddMgr.declare(propName)
        return self.ddMgr.var(propName)
...
def parseNonTemporalSubformula(node,uvw):
    ...
    if node.value==NodeTypes.PROPOSITION:
        return uvw.getBDDFromPropositionName(node.children)
...
def constructUVW(node,ddMgr=None):
