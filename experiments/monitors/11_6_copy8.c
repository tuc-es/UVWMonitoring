...
const uint8_t lut1[64] = {0x4,0x5,0xc,0x4,0x4,0x1,0xc,0x4,
    0x14,0x15,0x18,0x14,0x4,0x5,0xc,0x4,
    0x6,0x3,0xe,0x6,0x6,0x3,0xe,0x6,
    0x16,0x13,0x1a,0x16,0x6,0x3,0xe,0x6,
    0x24,0x25,0x28,0x24,0x24,0x21,0x28,0x24,
    0x34,0x35,0x38,0x34,0x24,0x25,0x28,0x24,
    0x4,0x5,0xc,0x4,0x4,0x1,0xc,0x4,
    0x14,0x15,0x18,0x14,0x4,0x5,0xc,0x4};
const uint8_t lut2[64] = {0x0,0x1,0x0,0x1,0x0,0x1,0x0,0x1,
    ...
    0x0,0x1,0x0,0x1,0x0,0x1,0x0,0x1,
    0x0,0x1,0x0,0x1,0x0,0x1,0x0,0x1};
const uint16_t lut3[64] = {0x8,0x8,0x481,0x481,0x1808,0x8,0x1c81,0x481,
    0x8,0x8,0x8,0x8,0x1808,0x8,0x1808,0x8,
    0xc,0xc,0x48c,0x485,0x1806,0x50c,0x1c86,0x585,
    0x68,0x8,0x68,0x8,0x1862,0x8,0x1862,0x8,
    0x6008,0x6008,0x6481,0x6481,0x7808,0x6008,0x7c81,0x6481,
    0x8,0x8,0x8,0x8,0x1808,0x8,0x1808,0x8,
    0x6014,0x660c,0x6494,0x6685,0x7816,0x670c,0x7c96,0x6785,
    0x68,0x8,0x68,0x8,0x1862,0x8,0x1862,0x8};
...
const uint16_t lut5[64] = {0x60,0x0,0x800,0x0,0x70,0x2d0,0x800,0x2d1,
...
void monitorUpdate(uint32_t apValues) {
  ...
  uint32_t lut2input = ((lut1output & 4)>0?1:0) | ((lut1output & 32)>0?2:0) | (apValues & 60);
  uint32_t lut2output = lut2[lut2input];
  uint32_t lut3input = (uvwState19>0?2:0) | (uvwState20>0?4:0) | ((apValues & 256)>0?16:0) | (uvwState21>0?32:0) | (lut1output & 9);
  uint32_t lut3output = lut3[lut3input];
  uint32_t lut4input = (uvwState18>0?16:0) | ((lut2output & 1)>0?32:0) | (lut0output & 1) | (lut1output & 2) | (lut3output & 12);
  uint32_t lut4output = lut4[lut4input];
  uint32_t lut5input = ((apValues & 64)>0?1:0) | ((apValues & 128)>0?2:0) | (uvwState7>0?4:0) | (uvwState8>0?8:0) | (uvwState15>0?16:0) | (uvwState16>0?32:0);
  ...
  uint32_t lut7input = (lut4output & 1) | (lut6output & 2) | (lut5output & 12) | (lut0output & 48);
  uint32_t lut7output = lut7[lut7input];
  uint32_t lut8input = (uvwState9>0?4:0) | ((lut7output & 1)>0?16:0) | (uvwState10>0?32:0) | (apValues & 3) | (lut6output & 8);
  uint32_t lut8output = lut8[lut8input];
  uint32_t lut9input = (lut5output & 3) | (lut0output & 12) | (lut6output & 16) | (lut8output & 32);
  ...
  uvwState7 = (lut5output & 64);
  uvwState8 = (lut5output & 512);
  ...
  uvwState15 = (lut5output & 2048);
  uvwState16 = (lut5output & 16384);
...
- LUT 1 has:
  ...
  -> Outputs new_n34 new_n52 new_n69 uvwState9incoming1 uvwState11incoming1 uvwState15incoming1
- LUT 2 has:
  -> Inputs new_n69 uvwState15incoming1 b1 b2 c1 c2
  -> Outputs new_n75
- LUT 3 has:
...
- LUT 7 has:
  -> Inputs new_n90 uvwState0incoming12 uvwState0incoming15 uvwState0incoming16 uvwState0incoming13 uvwState0incoming14
  -> Outputs new_n95
- LUT 8 has:
  -> Inputs a1 a2 uvwState9 uvwState0incoming11 new_n95 uvwState10
